...
class _WidgetFormState extends State<WidgetForm> {
  ...
  Widget build(BuildContext context) {
    return Form(key: _formKey, child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, mainAxisSize: MainAxisSize.min, children: _buildFormWidgets()));
  }
  ...
  List<Widget> _buildFormWidgets() {
    ...
    for (var field in widget.routeData.form.fields) {
      ...
      } else if (field.source == FormItemSource.listed) {
        if (field.values == null) {
          throw Exception("The field ${field.key} has a source listed, but has no values defined.");
        }
        _addListedSelection(field, children, field.values!);
  ...
  void _addListedSelection(Field field, List<Widget> children, List<FormItemKeyValue> options) {
    // TODO: WidgetNameValueSelection cannot be used here. Same for RouteData.
    children.add(WidgetNameValueSelection(field: field, valuesGenerator: ValueGeneratorFormItemKeyValue(options)));
  }
  ...
  bool _addBranchSelection(bool includeSizeBox, Field? branchField, Field field, Field? bankField, List<Widget> children) {
    ...
    if (bankField.value.value != null && bankField.value.value != "")  {
      ...
      if (branchesForBank == null) {
        throw Exception("Cannot find the branches for the bank ${bankField.getValueAs<ListItemNameValue>().toString()}.");
      }
      children.add(WidgetNameValueSelection(
          field: field, valuesGenerator: ValueGeneratorBranch(branchesForBank), textController: _branchEditingController, valueSelectedCallback: null));
    }
